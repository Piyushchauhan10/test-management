...
export function RichTextEditor({
  ...
  useEffect(() => {
    ...
    const belowTop =
      editorRect.top +
      mentionState.anchorTop +
      mentionState.anchorHeight +
      verticalGap;
    const canRenderBelow =
      belowTop + menuHeight <= window.innerHeight - horizontalPadding;
    const aboveTop =
      editorRect.top + mentionState.anchorTop - menuHeight - verticalGap;

    setMenuPosition({
      top: canRenderBelow ? belowTop : Math.max(horizontalPadding, aboveTop),
      left: nextLeft,
      width: resolvedWidth,
  ...
  useEffect(() => {
    ...
    quill.clipboard.dangerouslyPasteHTML(
      normalizeHtml(initialValueRef.current),
    );
    ...
    const handleTextChange = () => {
      ...
      onChangeRef.current(normalizeHtml(cleanHtml));
      scheduleMentionSync();
    };
    ...
    quill.keyboard.addBinding({ key: "Enter" }, () => {
      ...
      const selectedItem =
        currentSuggestions[highlightedIndexRef.current] ||
              ...
              {filteredMentions.map((item, index) => (
                <button
                  ...
                  onMouseDown={(event) => {
                    ...
                    quill.insertText(
                      currentMention.triggerIndex + 1,
                      " ",
                      "user",
                    );
                    quill.setSelection(
                      currentMention.triggerIndex + 2,
                      0,
                      "user",
                    );
                    setMentionState(null);
